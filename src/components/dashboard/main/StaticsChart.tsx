...
const SubmissionInsightsChart: React.FC = () => {
        ...
        if (active && payload && payload.length) {
            ...
            return (
                <div className="bg-gray-800 text-white text-xs px-3 py-2 rounded shadow-lg">
                    <div className="font-semibold">{Math.round(payload[0].value)} submissions</div>
    ...
    // Custom bar shape to handle highlighting
    const CustomBar = (props: any) => {
        const { fill, ...rest } = props;
        const isHighlighted = props.payload?.isHighlighted;
        const barColor = isHighlighted ? '#008402' : '#2489B0';
